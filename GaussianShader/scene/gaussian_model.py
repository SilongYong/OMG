...
class MLPOpacity_Fea(torch.nn.Module):
    def __init__(self,inChanel, featureC=128):
        super(MLPOpacity_Fea, self).__init__()
...
class GaussianModel:
    def __init__(self, sh_degree : int, brdf_dim : int, brdf_mode : str, brdf_envmap_res: int):
        ...
        self._opacity_mlp = MLPOpacity_Fea(3 + 3 + 1)
    ...
    def get_opacity(self):
        diffuse_dim = self._features_dc.shape[-1]
        roughness_dim = self._roughness.shape[-1]
        specular_dim = self._specular.shape[-1]

        all_features = torch.cat(
            [
                self.get_diffuse.view(-1, diffuse_dim),
                self.get_roughness.view(-1, roughness_dim),
                self.get_specular.view(-1, specular_dim),
            ],
            dim=-1
        )

        return self._opacity_mlp(all_features) * self.opacity_activation(self._opacity)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self._opacity_mlp = self._opacity_mlp.requires_grad_(True).to(self._xyz.device)
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': self._opacity_mlp.parameters(), "lr": training_args.opacity_mlp_lr, "name": "opacity_mlp"},
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.opacity_activation(self._opacity), torch.ones_like(self.opacity_activation(self._opacity))*0.01))
