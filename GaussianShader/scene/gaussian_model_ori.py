...
class MLPOpacity_Fea(torch.nn.Module):
    def __init__(self,inChanel, featureC=128):
        ...
        # torch.nn.init.constant_(self.mlp[-1].bias, -2.197)
        # torch.nn.init.constant_(self.mlp[-1].weight, 0)
        # torch.nn.init.constant_(self.mlp[-1].bias, -2.197) # so far the best practice
        torch.nn.init.constant_(self.mlp[-1].bias, 0.0)
        # torch.nn.init.constant_(self.mlp[0].bias, 0.01)
        # torch.nn.init.constant_(self.mlp[2].bias, 0.01)
...
class GaussianModel:
    def __init__(self, sh_degree : int, brdf_dim : int, brdf_mode : str, brdf_envmap_res: int):
        ...
        # self._opacity_mlp = MLPOpacity_Fea(3*(self.max_sh_degree + 1)**2 + 3 + 3 + 1 + 1)
        self._opacity_mlp = None
