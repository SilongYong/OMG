...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_dir", type=str, help="The path to the output directory that stores the normal ground truth.")
    args = parser.parse_args()

    output_dir = args.output_dir

    gt_normal_dirs = os.path.join(args.gt_dir, "test")
    test_dirs = glob.glob(os.path.join(args.test_dir, "test", "ours_30000", "renders", "*.png"))
    test_dirs.sort()
    ...
    for test_dir in tqdm(test_dirs):
        test_id = int(os.path.split(test_dir)[-1].split(".")[0])
        normal_gt_path = os.path.join(gt_normal_dirs, f"r_{test_id}_normal.png")
